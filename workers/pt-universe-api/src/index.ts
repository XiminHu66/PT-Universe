...
async function lookupCover(title:string){try{const r=await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(title)}&country=JP&media=music&limit=1`);const j=await r.json<any>();return j.results?.[0]?.artworkUrl100?.replace('100x100','300x300')||''}catch{return ''}}
async function refreshMusic(env:Env,sharedBrowser?:Browser){
  const old=await previous<any>(env,'music.json',{});
  const scrape=async(browser:Browser)=>{
    const page=await browser.newPage(),sources:Record<string,Json>={};let weekly:any[]=[],recent:any[]=[];
    try{await page.goto('https://www.billboard-japan.com/charts/detail?a=hot100',{waitUntil:'domcontentloaded',timeout:25_000});weekly=await page.locator('td.name_td').evaluateAll(els=>els.slice(0,30).map((el:any,index:number)=>({rank:index+1,title:(el.querySelector('p.musuc_title')?.textContent||'').trim(),artist:(el.querySelector('p.artist_name')?.textContent||'').trim()})).filter((x:any)=>x.title));sources.billboard_japan={label:'Billboard JAPAN Hot 100',ok:true,count:weekly.length}}catch(e){weekly=old.weekly_chart||[];sources.billboard_japan={label:'Billboard JAPAN Hot 100',ok:false,count:weekly.length,error:String(e),fallback:true}}
    try{await page.goto('https://kworb.net/youtube/insights/jp.html',{waitUntil:'domcontentloaded',timeout:25_000});recent=await page.locator('#weeklytable tbody tr').evaluateAll(els=>els.slice(0,30).map((el:any,index:number)=>{const c=[...el.querySelectorAll('td')].map((x:any)=>x.textContent.trim());const a=el.querySelector('a');return {rank:Number(c[0])||index+1,title:(a?.textContent||c[2]||'').trim(),artist:'',movement:c[1]||'',weeks:Number(c[3])||null,peak:Number(c[4])||null,streams:c[5]||'',url:a?.href||''}}).filter((x:any)=>x.title));sources.youtube_japan_recent={label:'YouTube Japan Weekly via Kworb',ok:true,count:recent.length}}catch(e){recent=old.recent_chart||old.recent_songs||[];sources.youtube_japan_recent={label:'YouTube Japan Weekly via Kworb',ok:false,count:recent.length,error:String(e),fallback:true}}finally{await page.close()}
    for(const song of recent.slice(0,12))if(!song.cover)song.cover=await lookupCover(song.title);
...
async function scrapeGames(browser:Browser,old:any,sources:Record<string,Json>){
  const page=await browser.newPage(),mobile:any[]=[];let consoleRows:any[]=[];
  const mobileSources=[
...
export default {
  ...
  async queue(batch:MessageBatch<RefreshMessage>,env:Env){
    for(const message of batch.messages){const {requestId,scope,limitKeys=[]}=message.body,start=Date.now();try{await env.DB.prepare('UPDATE refresh_runs SET status=?,started_at=? WHERE request_id=?').bind('running',now(),requestId).run();const result=await runRefresh(env,scope);await env.DB.prepare('UPDATE refresh_runs SET status=?,completed_at=?,duration_ms=?,result_json=? WHERE request_id=?').bind('success',now(),Date.now()-start,JSON.stringify(result),requestId).run();message.ack()}catch(e){console.error('refresh_failed',{requestId,scope,error:String(e)});await env.DB.prepare('UPDATE refresh_runs SET status=?,completed_at=?,duration_ms=?,error=? WHERE request_id=?').bind('failed',now(),Date.now()-start,String(e),requestId).run();for(const key of limitKeys)await env.DB.prepare('DELETE FROM refresh_limits WHERE limit_key=?').bind(key).run();message.retry()}}
  }
