...
async function refreshDefaultFeed(source,force=false){try{const items=await parseCustomFeed(await fetchText(source.url,source.id,force),source);return items.length?{source,items}:null}catch{return null}}
async function refreshDefaultFeeds(force=false){const sources=(data.sources||[]).filter(x=>!hiddenIds.has(x.id)),results=await Promise.all(sources.map(source=>refreshDefaultFeed(source,force))),successful=results.filter(Boolean);for(const result of successful){data.items=[...result.items,...data.items.filter(x=>x.source_id!==result.source.id)];data.sources=data.sources.map(x=>x.id===result.source.id?{...x,status:force?'live':'cached',item_count:result.items.length,error:''}:x)}if(force&&successful.length)manualRefreshAt=new Date();return{sourceCount:successful.length,totalSources:sources.length,itemCount:successful.reduce((sum,x)=>sum+x.items.length,0)}}
