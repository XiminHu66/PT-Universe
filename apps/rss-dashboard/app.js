...
function renderManager(){
  ...
  $('#sourceManager').innerHTML=[...defaults.map(x=>`<div class="source-row" style="--dot:${sourceColor(x.id)}"><div><i></i><span><strong>${esc(x.name)}</strong><small>${esc(categoryInfo(x.category).name)} · ${x.status==='live'?'刚刚手动刷新':x.status==='ok'?'抓取正常':x.status==='cached'?'使用上次缓存':'等待首次抓取'}</small></span></div><button type="button" class="${hiddenIds.has(x.id)?'':'on'}" data-toggle-source="${esc(x.id)}">${hiddenIds.has(x.id)?'已关闭':'已启用'}</button></div>`),...customFeeds.map(x=>`<div class="source-row" style="--dot:${sourceColor(x.id)}"><div><i></i><span><strong>${esc(x.name)}</strong><small>${esc(categoryInfo(x.category).name)} · 本机订阅</small></span></div><button type="button" class="remove" data-remove-source="${esc(x.id)}">移除</button></div>`)].join('')||'<p class="dialog-note">尚未载入订阅源。</p>';
...
async function fetchText(url,sourceId='',force=false){let lastError;const proxy=proxyBase();if(proxy&&sourceId){try{return await fetchCandidate(`${proxy}/feed/${encodeURIComponent(sourceId)}${force?'?refresh=1':''}`,22000)}catch(error){lastError=error}}const encoded=encodeURIComponent(url),candidates=[url,`https://api.allorigins.win/raw?url=${encoded}`,`https://api.codetabs.com/v1/proxy?quest=${encoded}`];for(const candidate of candidates){try{return await fetchCandidate(candidate)}catch(error){lastError=error}}throw lastError||new Error('feed unavailable')}
...
async function refreshDefaultFeed(source){try{const items=await parseCustomFeed(await fetchText(source.url,source.id,true),source);return items.length?{source,items}:null}catch{return null}}
async function refreshDefaultFeeds(){const sources=(data.sources||[]).filter(x=>!hiddenIds.has(x.id)),results=await Promise.all(sources.map(refreshDefaultFeed)),successful=results.filter(Boolean);for(const result of successful){data.items=[...result.items,...data.items.filter(x=>x.source_id!==result.source.id)];data.sources=data.sources.map(x=>x.id===result.source.id?{...x,status:'live',item_count:result.items.length,error:''}:x)}if(successful.length)manualRefreshAt=new Date();return{sourceCount:successful.length,totalSources:sources.length,itemCount:successful.reduce((sum,x)=>sum+x.items.length,0)}}
...
function applyReaderRatio(value=store.get(KEYS.readerRatio,42),persist=false){const ratio=Math.min(50,Math.max(32,Number(value)||42)),available=Math.max(0,innerWidth-246),maxWidth=Math.min(680,Math.max(340,available-420)),width=Math.min(maxWidth,Math.max(340,Math.round(available*ratio/100)));document.documentElement.style.setProperty('--reader',`${width}px`);if(persist)store.set(KEYS.readerRatio,ratio);const input=$('#readerWidthInput'),output=$('#readerWidthOutput');if(input)input.value=ratio;if(output)output.textContent=`${ratio}%`}
function closeSidebar(){$('#sidebar').classList.remove('open');$('#scrim').classList.remove('show')}
function openManage(){renderManager();renderProxySettings();$('#manageDialog').showModal();closeSidebar()}
...
function syncStatus(){if(manualRefreshAt){$('#syncDot').className='ok';$('#syncText').textContent=`手动刷新于 ${fullDate(manualRefreshAt)}`;return}const stamp=data.generated_at?new Date(data.generated_at):null;if(stamp&&Number.isFinite(stamp.getTime())){$('#syncDot').className='ok';$('#syncText').textContent=`仓库更新于 ${fullDate(stamp)}`}else{$('#syncDot').className='error';$('#syncText').textContent='等待首次数据抓取'}}
async function loadData(){const btn=$('#refreshButton');btn.classList.add('loading');try{const response=await fetch(DATA_URL);if(!response.ok)throw new Error();const payload=await response.json();if(Array.isArray(payload.items)){data=payload;syncStatus();await refreshCustomFeeds();renderAll()}}catch{$('#syncDot').className='error';$('#syncText').textContent='数据缓存暂时不可用';renderAll()}finally{btn.classList.remove('loading')}}
async function refreshNow(){const btn=$('#refreshButton');if(btn.classList.contains('loading'))return;btn.classList.add('loading');btn.disabled=true;toast(proxyBase()?'正在通过 Cloudflare 刷新全部 RSS…':'正在直接读取 RSS 源…');try{try{const response=await fetch(`${DATA_URL}?t=${Date.now()}`,{cache:'no-store'}),payload=await response.json();if(response.ok&&Array.isArray(payload.items))data=payload}catch{}const live=await refreshDefaultFeeds();await refreshCustomFeeds();syncStatus();renderAll();if(live.sourceCount===live.totalSources)toast(`全部 ${live.sourceCount} 个来源已实时刷新 · ${live.itemCount} 篇`);else if(live.sourceCount)toast(`实时更新 ${live.sourceCount} 个来源；其余 ${live.totalSources-live.sourceCount} 个沿用缓存`);else toast('已检查仓库缓存；RSS 源当前无法直连')}finally{btn.classList.remove('loading');btn.disabled=false}}
...
setTheme(store.get(KEYS.theme,matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light'));applyReaderRatio();renderAll();loadData();
