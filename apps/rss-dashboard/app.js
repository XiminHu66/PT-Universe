...
const KEYS={read:'pt.rss.read',saved:'pt.rss.saved',hidden:'pt.rss.hidden',theme:'pt.rss.theme',custom:'pt.rss.custom',customItems:'pt.rss.customItems',readerRatio:'pt.rss.readerRatio'};
...
async function fetchText(url){const encoded=encodeURIComponent(url),candidates=[url,`https://api.allorigins.win/raw?url=${encoded}`,`https://api.codetabs.com/v1/proxy?quest=${encoded}`];let lastError;for(const candidate of candidates){try{return await fetchCandidate(candidate)}catch(error){lastError=error}}throw lastError||new Error('feed unavailable')}
...
async function refreshDefaultFeed(source){try{const items=await parseCustomFeed(await fetchText(source.url),source);return items.length?{source,items}:null}catch{return null}}
