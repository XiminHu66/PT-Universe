...
async function fetchCandidate(url,timeout=10000){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeout);try{const response=await fetch(url,{signal:ctl.signal,cache:'no-store',headers:{Accept:'application/rss+xml, application/atom+xml, application/xml, text/xml, */*'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=await response.text();if(!text.trim())throw new Error('empty response');return{text,meta:{cache:response.headers.get('X-RSS-Cache')||'DIRECT',fetchedAt:response.headers.get('X-RSS-Fetched-At')||new Date().toISOString()}}}finally{clearTimeout(timer)}}
...
function healthError(error){const value=String(error?.message||error||'读取失败').replace(/^upstream_/,'源站 HTTP ').replace('Failed to fetch','网络或源站拒绝访问');return value.slice(0,72)}
