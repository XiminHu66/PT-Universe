...
const KEYS={read:'pt.rss.read',saved:'pt.rss.saved',hidden:'pt.rss.hidden',deleted:'pt.rss.deleted',sourceOverrides:'pt.rss.sourceOverrides',theme:'pt.rss.theme',custom:'pt.rss.custom',customItems:'pt.rss.customItems',readerRatio:'pt.rss.readerRatio',fontScale:'pt.rss.fontScale',proxyUrl:'pt.rss.proxyUrl'};
...
function renderNavigation(){
  ...
  $('#categoryNav').innerHTML=Object.entries(CATEGORIES).map(([id,c])=>{const total=items.filter(x=>x.category===id).length,count=unreadItems.filter(x=>x.category===id).length;if(!total&&id==='other')return '';return `<button class="feed-item ${category===id?'active':''}" data-category="${id}" style="--dot:${c.color}"><i></i><span>${c.icon} ${c.name}</span><em title="未读文章数">${count}</em></button>`}).join('');
...
function sourceStatus(source){if(source.custom)return'本机订阅';if(sourceOverrides[source.id])return source.proxy_id===false?'已修改 · 本机读取':'已修改 · Cloudflare 缓存';return source.status==='live'?'刚刚手动刷新':source.status==='ok'?'抓取正常':source.status==='cached'?'Cloudflare 缓存':'等待首次抓取'}
function sourceRow(source){return `<div class="source-row" style="--dot:${sourceColor(source.id)}"><div class="source-info"><i></i><span><strong>${esc(source.name)}</strong><small>${esc(categoryInfo(source.category).name)} · ${esc(sourceStatus(source))}</small></span></div><div class="source-actions"><button type="button" class="${hiddenIds.has(source.id)?'':'on'}" data-toggle-source="${esc(source.id)}">${hiddenIds.has(source.id)?'已关闭':'已启用'}</button><button type="button" data-edit-source="${esc(source.id)}">修改</button><button type="button" class="remove" data-delete-source="${esc(source.id)}">删除</button></div></div>`}
...
function resetSourceForm(){editingSourceId=null;$('#sourceFormTitle').textContent='添加本机订阅';$('#newFeedName').value='';$('#newFeedUrl').value='';$('#newFeedCategory').value='tech';$('#addFeedButton').textContent='添加并尝试读取';$('#cancelEditButton').hidden=true}
function startSourceEdit(id){const source=allSources().find(x=>x.id===id);if(!source)return;editingSourceId=id;$('#sourceFormTitle').textContent='修改订阅源';$('#newFeedName').value=source.name;$('#newFeedUrl').value=source.url;$('#newFeedCategory').value=source.category;$('#addFeedButton').textContent='保存修改';$('#cancelEditButton').hidden=false;$('#sourceForm').scrollIntoView({block:'nearest'})}
async function saveSource(){const name=$('#newFeedName').value.trim(),url=safeUrl($('#newFeedUrl').value.trim()),cat=$('#newFeedCategory').value;if(!url)return toast('请输入完整的 RSS 地址');if(allSources().some(x=>x.id!==editingSourceId&&x.url===url))return toast('这个订阅已经存在');if(!editingSourceId){const source={id:`custom-${hash(url)}`,name:name||new URL(url).hostname.replace(/^www\./,''),url,category:cat,custom:true};customFeeds.push(source);store.set(KEYS.custom,customFeeds);resetSourceForm();renderManager();const ok=await refreshCustomFeed(source,true);renderAll();toast(ok?'订阅已添加并读取':'已保存订阅，但当前网络无法读取');return}const current=allSources().find(x=>x.id===editingSourceId);if(!current)return resetSourceForm();const updated={...current,name:name||new URL(url).hostname.replace(/^www\./,''),url,category:cat};if(current.custom){customFeeds=customFeeds.map(x=>x.id===current.id?{...updated,custom:true}:x);customItems=customItems.map(x=>x.source_id===current.id?{...x,source:updated.name,category:updated.category}:x);store.set(KEYS.custom,customFeeds);store.set(KEYS.customItems,customItems)}else{sourceOverrides[current.id]={name:updated.name,url:updated.url,category:updated.category};store.set(KEYS.sourceOverrides,sourceOverrides);data.items=data.items.map(x=>x.source_id===current.id?{...x,source:updated.name,category:updated.category}:x)}resetSourceForm();renderManager();renderAll();let ok=false;if(current.custom)ok=await refreshCustomFeed({...updated,custom:true},true);else{const preferred=defaultSources().find(x=>x.id===current.id),result=preferred&&await refreshDefaultFeed(preferred,true);if(result){data.items=[...result.items,...data.items.filter(x=>x.source_id!==current.id)];data.sources=data.sources.map(x=>x.id===current.id?{...x,status:'live',item_count:result.items.length,error:''}:x);ok=true}}renderManager();renderAll();toast(ok?'订阅已修改并刷新':'订阅已修改；当前无法读取新地址')}
function removeSource(id){const source=allSources().find(x=>x.id===id);if(!source||!confirm(`删除订阅“${source.name}”吗？`))return;if(source.custom){customFeeds=customFeeds.filter(x=>x.id!==id);customItems=customItems.filter(x=>x.source_id!==id);store.set(KEYS.custom,customFeeds);store.set(KEYS.customItems,customItems)}else{deletedIds.add(id);delete sourceOverrides[id];saveSet(KEYS.deleted,deletedIds);store.set(KEYS.sourceOverrides,sourceOverrides)}hiddenIds.delete(id);saveSet(KEYS.hidden,hiddenIds);if(feed===id)feed=null;if(editingSourceId===id)resetSourceForm();renderManager();renderAll();toast('订阅已删除')}
function restoreDefaultSources(){deletedIds.clear();saveSet(KEYS.deleted,deletedIds);renderManager();renderAll();toast('已恢复默认订阅源')}

async function fetchCandidate(url,timeout=10000){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeout);try{const response=await fetch(url,{signal:ctl.signal,cache:'no-store',headers:{Accept:'application/rss+xml, application/atom+xml, application/xml, text/xml, */*'}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=await response.text();if(!text.trim())throw new Error('empty response');return text}finally{clearTimeout(timer)}}
async function fetchText(url,sourceId='',force=false){let lastError;const proxy=proxyBase();if(proxy&&sourceId)return fetchCandidate(`${proxy}/feed/${encodeURIComponent(sourceId)}${force?'?refresh=1':''}`,22000);const encoded=encodeURIComponent(url),candidates=[url,`https://api.allorigins.win/raw?url=${encoded}`,`https://api.codetabs.com/v1/proxy?quest=${encoded}`];for(const candidate of candidates){try{return await fetchCandidate(candidate)}catch(error){lastError=error}}throw lastError||new Error('feed unavailable')}
...
async function refreshCustomFeed(source,quiet=false){try{const xml=await fetchText(source.url),items=await parseCustomFeed(xml,source);customItems=[...items,...customItems.filter(x=>x.source_id!==source.id)].slice(0,500);store.set(KEYS.customItems,customItems);if(!quiet)toast(`${source.name}：读取 ${items.length} 篇`);return true}catch(error){if(!quiet)toast(`${source.name} 暂时无法读取`);return false}}
async function refreshCustomFeeds(){const sources=customFeeds.filter(x=>!hiddenIds.has(x.id));if(!sources.length)return;await Promise.all(sources.map(x=>refreshCustomFeed(x,true)))}

async function refreshDefaultFeed(source,force=false){try{const proxyId=source.proxy_id===false?'':source.id,items=await parseCustomFeed(await fetchText(source.url,proxyId,force),source);return items.length?{source,items}:null}catch{return null}}
async function refreshDefaultFeeds(force=false){const sources=defaultSources().filter(x=>!hiddenIds.has(x.id)),results=await Promise.all(sources.map(source=>refreshDefaultFeed(source,force))),successful=results.filter(Boolean);for(const result of successful){data.items=[...result.items,...data.items.filter(x=>x.source_id!==result.source.id)];data.sources=data.sources.map(x=>x.id===result.source.id?{...x,status:force?'live':'cached',item_count:result.items.length,error:''}:x)}if(force&&successful.length)manualRefreshAt=new Date();return{sourceCount:successful.length,totalSources:sources.length,itemCount:successful.reduce((sum,x)=>sum+x.items.length,0)}}
...
async function loadCloudflareStatus(){const proxy=proxyBase();if(!proxy)return;try{const response=await fetch(`${proxy}/health?t=${Date.now()}`,{cache:'no-store'}),payload=await response.json();if(response.ok&&payload?.last_refresh)data.generated_at=payload.last_refresh}catch{}}
...
async function refreshNow(){const btn=$('#refreshButton');if(btn.classList.contains('loading'))return;btn.classList.add('loading');btn.disabled=true;toast(proxyBase()?'正在通过 Cloudflare 即时刷新全部 RSS…':'正在直接读取 RSS 源…');try{const live=await refreshDefaultFeeds(true);await refreshCustomFeeds();syncStatus();renderAll();if(live.sourceCount===live.totalSources)toast(`全部 ${live.sourceCount} 个来源已实时刷新 · ${live.itemCount} 篇`);else if(live.sourceCount)toast(`实时更新 ${live.sourceCount} 个来源；其余 ${live.totalSources-live.sourceCount} 个沿用 Cloudflare 缓存`);else toast('RSS 源当前无法刷新，已保留 Cloudflare 缓存')}finally{btn.classList.remove('loading');btn.disabled=false}}
...
setTheme(store.get(KEYS.theme,matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light'));applyFontScale();applyReaderRatio();renderAll();loadData();
