...
async function load(){
  ...
  const [feed,library,site,news,content,music,games]=await Promise.all([
    ...
    loadJSON('data/music.json',{generated_at:null,weekly_chart:[],new_releases:[],ricecake_charts:[],sources:{}}),
...
function renderMusic(){
  const chart=state.music?.weekly_chart||[];const ricecake=state.music?.ricecake_charts||[];const releases=state.music?.recent_songs||state.music?.new_releases||[];const q=state.query.toLowerCase();
  $('#musicChartCount').textContent=chart.length;$('#musicRicecakeCount').textContent=ricecake.length;$('#musicReleaseCount').textContent=releases.length;$('#followedArtistCount').textContent=state.followedArtists.length;$('#musicChartDate').textContent=state.music?.chart_date?`榜单公布：${state.music.chart_date}`:'每周更新';
  ...
  const rc=ricecake.filter(x=>!q||`${x.title} ${x.period||''}`.toLowerCase().includes(q));
  $('#musicRicecakeGrid').innerHTML=rc.length?rc.map((x,i)=>`<a class="ricecake-chart-card" href="${esc(x.url||x.playlist_url||'#')}" target="_blank" rel="noopener"><div class="ricecake-thumb-wrap">${imageHTML({cover:x.thumbnail,type:'music',className:'ricecake-thumb',placeholderClass:'ricecake-thumb ricecake-placeholder',placeholderText:'▶'})}<span class="ricecake-play">▶</span><span class="ricecake-week">${i===0?'最新':'近月'}</span></div><div class="ricecake-copy"><span>${esc(x.period||relativeTime(x.published_at)||'近月周榜')}</span><h4>${esc(x.title)}</h4><p>${esc(x.source_label||'RICECAKE · J-Pop Weekly Chart')}</p></div></a>`).join(''):`<div class="empty music-empty">暂无 Ricecake 近月周榜。<small>下一次刷新 Action 会再次检查官方 YouTube RSS。</small></div>`;
