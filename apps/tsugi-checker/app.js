...
function renderMusic(){
  ...
  const rc=recentChart.filter(x=>!q||`${x.title} ${x.artist}`.toLowerCase().includes(q));
  $('#musicRecentChartList').innerHTML=rc.length?rc.map(x=>`<article class="music-chart-row"><div class="music-rank">${esc(x.rank)}</div>${imageHTML({cover:x.artwork,type:'music',className:'music-cover',placeholderClass:'music-cover cover-placeholder',placeholderText:'♪'})}<div class="music-track-copy"><h4>${esc(x.title)}${x.is_new?'<span class="chart-new-badge">NEW</span>':''}</h4><p>${esc(x.artist)} · 在榜 ${esc(x.weeks)} 周 · 峰值 #${esc(x.peak)}${x.rank_change?` · ${x.is_new?'新进榜':`较上周 ${esc(x.rank_change)}`}`:''}${x.streams?` · ${esc(x.streams)} 播放`:''}</p></div><div class="music-row-actions">${artistFollowButton(x.artist)}<a class="music-service-link yt" href="${esc(x.youtube_music_url||youtubeMusicSearch(x.title,x.artist))}" target="_blank" rel="noopener">YouTube Music ↗</a><a class="music-service-link" href="${esc(x.url||'#')}" target="_blank" rel="noopener">YouTube ↗</a></div></article>`).join(''):`<div class="empty music-empty">暂无近期热榜。<small>下一次刷新 Action 会重新抓取 YouTube Japan 周榜。</small></div>`;
...
$$('[data-game-view]').forEach(b=>b.onclick=()=>{state.gameView=b.dataset.gameView;$$('[data-game-view]').forEach(x=>x.classList.toggle('active',x===b));$$('.game-pane').forEach(x=>x.classList.remove('active'));$(state.gameView==='prices'?'#pc-prices':`#game${state.gameView==='mobile'?'Mobile':state.gameView==='pc'?'Pc':'Console'}Pane`).classList.add('active');resetTabScroll()});
